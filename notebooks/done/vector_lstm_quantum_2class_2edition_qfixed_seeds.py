...
checkpoint_filepath = 'quantum_lstm_2class_2edition_qfixed/best_model_weights.pth'
path = 'quantum_lstm_2class_2edition_qfixed'
...
X_train, X_test, y_train, y_test = prepare_train_test_datasets(resized_camera_drawings, resized_cellphone_drawings,
                                                               test_size=0.2)
...
for iteration in range(100):
    ...
    for epoch in range(50):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), f'quantum_lstm_2class_2edition_qfixed/best_model_iteration_{iteration + 1}.pth')
        ...
        params_dict = {name: param.data.cpu().numpy() for name, param in model.named_parameters() if
                       param.requires_grad}
        np.savez(f'quantum_lstm_2class_2edition_qfixed/parameters_iteration_{iteration + 1}_epoch_{epoch + 1}.npz',
                 **params_dict)
