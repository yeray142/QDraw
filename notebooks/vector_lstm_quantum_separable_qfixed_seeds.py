...
for iteration in range(100):
    ...
    for epoch in range(50):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), f'separable/best_model_iteration_{iteration + 1}.pth')
        ...
        params_dict = {name: param.data.cpu().numpy() for name, param in model.named_parameters() if
                       param.requires_grad}
        np.savez(f'separable/parameters_iteration_{iteration + 1}_epoch_{epoch + 1}.npz',
                 **params_dict)

    plot_graphs(loss_list, val_loss_list, accuracy_list, val_accuracy_list, iteration, output_dir='separable/plots')
